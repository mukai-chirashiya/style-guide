...
### ローカルルールも尊重する
- コードを書く前にすでに書かれているコードを眺めて、使われている命名規則やコーディングスタイルを理解するように務める。<br>
...
タブはインデントに使用しない。<br>
※Visual Studio Codeでのテキストエディタではネストの種類(スペースorタブ)や大きさ(何個分)か変更できる。<br>
ネストの種類にスペースがされている場合は、TABキーを押してもネストした部分にはスペースが入力される。
...
- 画像のサイズ・形式について
  ...
  - ビットマップ形式の画像で書き出す場合に、高解像度ディスプレイに対応するためにできれば2xの解像度で書き出す。
...
#### 画像の形式
デザインカンプから画像を書き出す場合は高解像度ディスプレイに対応するために2xの解像度で書き出すことが望ましい。
...
このCSSスタイルガイドもこのゴールを満たすCSSを書くことを目指している。
...
この問題は、特定のCSS設計手法を導入することによって解決できる。CSS設計手法とは、CSSの設計に対して一連のルールを定めたもの。<br>
CSS設計手法の代表的なものには、OOCSS(オーオーシーエスエス)、SMACSS(スマックス)、BEM(ベム)と呼ばれるものがあるが、本スタイルガイドではBEMを推奨の設計手法とする。<br>
...
Elementはネストして利用することができる。
Elementは必ず何らかのBlockに属するものとして定義される。そのため、`block__element1__element2`といった、エレメントが何か他のエレメントに属する意味になるようなネーミングはすべきではない。
```html
<!--
