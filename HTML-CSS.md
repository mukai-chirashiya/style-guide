...
## はじめに
HTML/CSSのコーディングは自由度が高いですが、その分簡単にカオス化してしまいます。<br>
一貫したルールや設計のもとでコードを書くことにより、コードの品質を一定以上にたもつことができ、保守や修正にかかる労力を大幅に削減することができます。<br>
また、コードを書く際の迷いが減るため開発スピードが向上します。

本コーディングガイドは以下のドキュメントや記事を参考に作成している。
...
CSSは記述の自由度が高いため、一定のルールに基づいて運用しないと簡単にカオス化してしまう。(“CSS is fragile.” （CSSは壊れやすい）という格言(?)が存在する。)<br>
この問題は、特定のCSS設計手法を導入することによって解決できる。CSS設計手法とは、CSSの書き方に対して、一連の規則を定めたものである。<br>
CSS設計手法の代表的なものには、OOCSS(オーオーシーエスエス)、SMACSS(スマックス)、BEM(ベム)と呼ばれるものがある。<br>
本スタイルガイドでは、BEMを推奨の設計手法とする。<br>
